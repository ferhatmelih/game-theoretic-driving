...
class ExperienceSource:
    ...
    def __init__(self, env, agent, steps_count=2, steps_delta=1, vectorized=False):
        ...
        self.total_soft_accidents = 0
        self.total_wrong_exits = 0
        self.init_vehicle_info = []
        self.init_input_state = []
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for env_idx, (env, action_n) in enumerate(zip(self.pool, grouped_actions)):
                if self.vectorized:
                    next_state_n, r_n, is_done_n, pygamesummary = env.step(action_n)
                else:
                    next_state, r, is_done, pygamesummary = env.step(action_n[0])
                ...
                for ofs, (action, next_state, r, is_done) in enumerate(zip(action_n, next_state_n, r_n, is_done_n)):
                    ...
                    if "total_hard_accidents" in pygamesummary:
                    	self.total_accidents = pygamesummary["total_hard_accidents"]

                    if "total_soft_accidents" in pygamesummary:
                    	self.total_soft_accidents = pygamesummary["total_soft_accidents"]
                    
                    if "total_wrong_exits" in pygamesummary:
                    	self.total_wrong_exits = pygamesummary["total_wrong_exits"]

                    if "init_vehicle_info" in pygamesummary:
                        self.init_vehicle_info = pygamesummary["init_vehicle_info"]

                    if "init_input_state" in pygamesummary:
                        self.init_input_state = pygamesummary["init_input_state"]

                    if "num_steps_taken" in pygamesummary:
                        self.num_steps_taken = pygamesummary["num_steps_taken"]
